...
// src/api/supabase.ts
var initSupabaseClient = (supabaseUrl, supabaseKey) => {
  return createClient(supabaseUrl, supabaseKey);
};

// src/main.ts
var BetterSelfPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.supabase = null;
  }
  ...
  async createMorningCheckIn() {
    ...
    if (!file) {
      ...
      new import_obsidian.Notice("\u{1F4C5} Morning check-in created!");
    }
    const leaf = this.app.workspace.getLeaf();
    await leaf.openFile(file);
  }
  async getMorningTemplate(date) {
    let models = ["Researcher", "Career Builder", "Poet", "Parent"];
    if (this.supabase) {
      const { data, error } = await this.supabase.from("temporal_models").select("name").order("name");
...
`;
  ...
  async createEveningReview() {
    ...
    if (!file) {
      new import_obsidian.Notice("\u26A0\uFE0F Create a morning check-in first!");
      return;
    }
    const leaf = this.app.workspace.getLeaf();
    await leaf.openFile(file);
    ...
    new import_obsidian.Notice("\u{1F4DD} Evening review ready!");
  }
  async generateTasks() {
    if (!this.supabase) {
      new import_obsidian.Notice("\u26A0\uFE0F Configure Supabase in settings first!");
      return;
    }
    try {
      new import_obsidian.Notice("\u{1F916} Generating tasks...");
      const file = this.app.workspace.getActiveFile();
      if (!file) {
        new import_obsidian.Notice("\u26A0\uFE0F Open a daily note first!");
        return;
      }
      ...
      const { data: goals, error: goalsError } = await this.supabase.from("goals").select("*").eq("temporal_model", temporalModel).eq("status", "active");
      if (goalsError) {
        throw goalsError;
      }
      ...
      if (view) {
        ...
        if (prioritiesIndex !== -1) {
          ...
          const taskList = tasks.slice(0, 3).map((t, i) => `${i + 1}. [ ] ${t.title} (${t.minutes}m) #${t.energy}`).join("\n");
          editor.replaceRange(taskList, { line: insertLine, ch: 0 });
        }
      }
      new import_obsidian.Notice(`\u2705 Generated ${tasks.length} tasks!`);
    } catch (error) {
      console.error("Error generating tasks:", error);
      new import_obsidian.Notice("\u274C Failed to generate tasks. Check console for details.");
    }
  }
  openFocusSession() {
    window.open(`${this.settings.webAppUrl}/focus`, "_blank");
    new import_obsidian.Notice("\u{1F3AF} Opening focus session...");
  }
  openInterviewPractice() {
    window.open(`${this.settings.webAppUrl}/interview`, "_blank");
    new import_obsidian.Notice("\u{1F3A4} Opening interview practice...");
  }
};
var BetterSelfSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Supabase URL").setDesc("Your Supabase project URL").addText(
      (text) => text.setPlaceholder("https://xxxxx.supabase.co").setValue(this.plugin.settings.supabaseUrl).onChange(async (value) => {
        this.plugin.settings.supabaseUrl = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Supabase API Key").setDesc("Your Supabase anon/public key").addText(
      (text) => text.setPlaceholder("eyJhbGc...").setValue(this.plugin.settings.supabaseKey).onChange(async (value) => {
        this.plugin.settings.supabaseKey = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Web App URL").setDesc("URL of the Better Self web application").addText(
      (text) => text.setPlaceholder("http://localhost:3000").setValue(this.plugin.settings.webAppUrl).onChange(async (value) => {
